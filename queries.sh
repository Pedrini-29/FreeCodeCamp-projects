...
echo  "$($PSQL "SELECT name FROM ( SELECT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE round='Eighth-Final' AND year=2014 UNION ALL SELECT name FROM games INNER JOIN teams ON games.OPPONENT_id = teams.team_id WHERE round='Eighth-Final' AND year=2014 ) g ORDER BY name")"
